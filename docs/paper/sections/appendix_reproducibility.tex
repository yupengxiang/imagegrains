...
Cellpose-SAM 模型 & \texttt{IG2\_full\_set\_cp\_SAM} & ImageGrains 2.0 预训练模型 & 比赛前固定版本 \\
...
系统的输出不是单一最终图片，而是一组从原始实例到场景报告的分层证据。表\ref{tab:appendix-outputs}给出主要文件与用途。文件名可能随场景 id 改变，但后缀和语义保持一致。
...
\texttt{*\_composite.png} / \texttt{*\_detection\_overview.png} & 原图与实例边界/标注叠加 & 快速发现 merge、split、漏检 \\
\texttt{*\_pred\_grains.csv} & 像素级面积、轴长、周长等 & 几何测量中间证据 \\
\texttt{*\_re\_scaled.csv} & 加入毫米轴长的逐颗粒表 & 粒径分析、尺度追溯 \\
\texttt{*\_particles\_annotated.csv} & $d_i$、形貌和异常标签等 & 单颗粒级结果追溯 \\
\texttt{*\_summary.json} & resolution、参数、$D_p$、粒级、形貌、异常汇总 & 场景级机器可读报告 \\
\texttt{*\_gsd\_comparison.png} & 数量型与质量代理级配图 & 解释统计口径差异 \\
\texttt{*\_axes\_overlay.png} & 颗粒长短轴和尺度叠加 & 物理尺寸目检 \\
\texttt{*\_shape\_*.png} & 各投影形貌类别高亮 & 形貌结果展示与人工核验 \\
\texttt{*\_anomaly\_*.png} & 异常类别高亮 & 大块/过小/疑似异常定位 \\
\bottomrule
\end{tabular}
\end{table}
