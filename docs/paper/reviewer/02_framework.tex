...
技术路线的选择需要同时满足三项约束：能够分离自然堆积中的接触颗粒；尽量不依赖现场重新标注训练；最终输出必须由可解释的物理量和统计规则构成。传统阈值/分水岭实现简单，但对背景、阴影和弱接触边界敏感；监督式实例分割可以针对赛题训练，却需要代表性实例标注并承担现场域变化风险；多视角或深度方案能够恢复更多三维信息，但设备、标定和现场布置复杂度更高。预训练颗粒实例模型则把最依赖大规模视觉数据的“边界识别”交给已有模型，同时允许赛题特有的毫米尺度与级配语义由显式规则完成。因此本文选择 ImageGrains 2.0 / Cellpose-SAM 作为上游基座，而不是从头训练一个端到端网络\cite{stringer2021cellpose,pachitariu2025cellposeSAM,mair2026imagegrains2}。
...
多视角/深度/点云 & 可恢复部分第三维 & 标定、硬件和部署成本高 & 物理信息更丰富，但不采用为当前主路线 \\
\bottomrule
\end{tabular}
\end{table}

本文系统采用单向、可追溯的数据链，如图\ref{fig:r-architecture}。在线处理从图像和尺度参考出发，依次得到实例掩膜、毫米级几何量、筛分等效粒径和质量代理级配；形貌与异常作为几何特征的并行分支，最终汇总为场景级报告。当前报告使用固定的 $\boldsymbol{\theta}=(1,0,0)$ 和 $\gamma=3$，因此运行时不存在依据输出结果临时调参的闭环。代码层保留了在存在外部机械筛分数据时重新估计 $\boldsymbol{\theta}$ 与 $\gamma$ 的可选接口，但该接口不属于本文现有实验结果。
...
\end{tikzpicture}
\caption{本文实际采用的主数据流。上游实例结果统一进入毫米几何表，下游级配、形貌和异常输出共享同一组可追溯颗粒特征。}
\label{fig:r-architecture}
\end{figure}

这种拆分体现三个工程原则。\textbf{泛化优先}意味着现场主要依赖预训练实例模型和标准化成像，而不是重新训练；\textbf{映射显式}意味着所有直接决定毫米量和级配口径的关系都有明确公式和固定参数，便于判断误差来自实例、尺度、粒径映射还是质量代理；\textbf{部署可控}意味着主链只有单目成像、尺度校验和固定配置推理，能够把主要现场时间留给质检和失败恢复。与直接从图像回归级配曲线的端到端方式相比\cite{coenen2022learningToSieve,buscombe2020sedinet}，该路线牺牲部分模型自由度，但换取了更强的物理可解释性、逐颗粒可追溯性和较低的现场数据依赖。

第 3 节讨论上游“图像如何变成可靠的毫米几何量”，第 4 节进一步说明“二维几何如何转换为筛级和质量代理统计”。这两个层次分别解决观测尺度与统计语义，是从实例识别走向可解释级配输出的关键连接。