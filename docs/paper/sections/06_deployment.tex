...
\begin{tabular}{>{\raggedright\arraybackslash}p{2.0cm}>{\raggedright\arraybackslash}p{5.0cm}>{\raggedright\arraybackslash}p{7.0cm}}
\toprule
模块 & 推荐条件 & 现场作用 \\
\midrule
...
表\ref{tab:r-sop}给出面向赛题 30~min 约束的现场时间预算。约 27~s/张的历史算法时序只覆盖计算阶段，完整操作还需要设备就位、尺度确认、图像质检、模型加载、结果复核和一次失败恢复。
...
\footnotesize
\renewcommand{\arraystretch}{1.12}
\begin{tabular}{>{\centering\arraybackslash}p{0.9cm}>{\raggedright\arraybackslash}p{3.5cm}>{\centering\arraybackslash}p{1.5cm}>{\raggedright\arraybackslash}p{7.4cm}}
\toprule
步骤 & 操作 & 预算 & 完成判据 \\
\midrule
...
\begin{tabular}{>{\raggedright\arraybackslash}p{3.0cm}>{\raggedright\arraybackslash}p{6.5cm}>{\raggedright\arraybackslash}p{7.0cm}}
\toprule
失败信号 & 优先检查 & 恢复原则 \\
\midrule
...
当大部分颗粒严重遮挡、参考物无法建立可信尺度，或实例结果出现固定流程无法恢复的大面积错误时，场景不进入主级配统计，避免在证据不足的场景上继续叠加后处理。
...
\footnotesize
\renewcommand{\arraystretch}{1.1}
\begin{tabular}{>{\raggedright\arraybackslash}p{2.7cm}>{\raggedright\arraybackslash}p{5.7cm}>{\raggedright\arraybackslash}p{7.0cm}}
\toprule
边界 & 主要影响 & 本文采用的处理方式 \\
\midrule
